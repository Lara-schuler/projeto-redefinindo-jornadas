...
class Usuario {
  ...
  static async buscarPorTelefone(telefone) {
    const [ddd, numero] = [telefone.substring(0, 2), telefone.substring(2)];
    const sql = 'SELECT * FROM telefone WHERE ddd = ? AND numero = ?';
  ...
  static async atualizarToken(email, token, expiration) {
    const sql = 'UPDATE pessoa SET reset_token = ?, token_expiration = ? WHERE email = ?';
  ...
  static async limparToken(email) {
    const sql = 'UPDATE pessoa SET reset_token = NULL, token_expiration = NULL WHERE email = ?';
  ...
  static async obterUsuarioPorId(id_pessoa) {
    ...
    console.log('Resultado da consulta de obterUsuarioPorId:', result);
